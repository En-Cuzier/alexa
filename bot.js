        ...
        if (config.WORKTYPE == 'public') {
            if (config.LANG == 'SI' || config.LANG == 'AZ') {
                ...
                if (commits.total === 0) {
                    ...
                    await conn.sendMessage(
                        conn.user.jid,
                        '```🧝‍♀️ යතාවත්කාලීන කිරීමට``` *.up now* ```භාවිතා කරන්න.```\n\n' + TEENU + '```', MessageType.text
                    ); 
                }
            }
            else { 
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '╭──────────────────●►\n│   *⦁◊═⦁QUEEN 👸 ALEXA⦁═◊⦁*\n│         ```working as public```\n│\n│ *🛡 This is your log number.*\n│ *🛡 Do not try any command in this*\n│ *chat.*\n │ *🛡 You can try any chat.*\n│ *🛡 Your bot working as public.*\n│ *🛡 If you want to change*\n│ *private*\n │ *mode,Try this command.*\n│\n│ ```.setvarWORK_TYPE:private```\n│\n│ *ᴛʜᴀɴᴋꜱ ꜰᴏʀ ᴜꜱɪɴɢ ǫᴜᴇᴇɴ ᴀʟᴇxᴀ*\n╰──────────────────●►\n', MessageType.text);
                }               
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
            ...
            if (config.LANG == 'SI' || config.LANG == 'AZ') { 
                ...
                if (config.FULLEVA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, ' ╭──────────────────●►\n│   *⦁◊═⦁QUEEN 👸 ALEXA⦁═◊⦁*\n│         ```working as private```\n│\n│ *🛡 This is your log number.*\n│ *🛡 Do not try any command in this │ chat.*\n│ *🛡 You can try any chat.*\n│ *🛡 Your bot working as private.*\n│ *🛡 If you want to change*\n│ *public*\n │ *mode,Try this command.*\n│\n│ ```.setvarWORK_TYPE:private```\n│\n│ *ᴛʜᴀɴᴋꜱ ꜰᴏʀ ᴜꜱɪɴɢ ǫᴜᴇᴇɴ ᴀʟᴇxᴀ*\n╰──────────────────●►', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
